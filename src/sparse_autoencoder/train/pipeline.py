...
from src.sparse_autoencoder.metrics.train.abstract_train_metric import TrainMetricData
from src.sparse_autoencoder.source_model.store_activations_hook import (
    store_activations_hook,
)
from src.sparse_autoencoder.tensor_types import BatchTokenizedPrompts, NeuronActivity
from src.sparse_autoencoder.train.abstract_pipeline import AbstractPipeline
from src.sparse_autoencoder.train.utils import get_model_device
...
class Pipeline(AbstractPipeline):
    ...
    def generate_activations(self, store_size: int) -> TensorActivationStore:
        ...
        with torch.no_grad():
            for batch in self.source_data:
                input_ids: BatchTokenizedPrompts = batch["input_ids"].to(
                    source_model_device
                )
    ...
    def train_autoencoder(
        ...
        learned_activations_fired_count: NeuronActivity = torch.zeros(
            self.autoencoder.n_learned_features,
            dtype=torch.int32,
        ...
        for store_batch in activations_dataloader:
            ...
            if (
                ...
                and self.total_training_steps % self.log_frequency == 0
            ):
                wandb.log(
