...
from src.sparse_autoencoder.metrics.metrics_container import (
    MetricsContainer,
    default_metrics,
)
    ...
    def save_checkpoint(self) -> None:
        ...
        if self.checkpoint_directory:
            file_path: Path = (
                ...
                / f"sae_state_dict-{self.total_training_steps}.pt"
            )
    ...
    def run_pipeline(
        ...
        with tqdm(
            ...
            for _ in range(0, max_activations, store_size):
                ...
                if (
                    last_resampled >= resample_frequency
                    and self.activation_resampler is not None
                ):
