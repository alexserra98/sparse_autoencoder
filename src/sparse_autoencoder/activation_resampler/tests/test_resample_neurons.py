    ...
    def test_sample_input_raises_value_error(self) -> None:
        ...
        with pytest.raises(
            ...
            ActivationResampler.sample_input(
                probabilities, input_activations, num_samples
            )
...
class TestResampleDeadNeurons:
    ...
    def test_no_changes_if_no_dead_neurons(self) -> None:
        ...
        assert (
            res.dead_decoder_weight_updates.numel() == 0
        ), "Should not have any dead neurons"
        assert (
            res.dead_encoder_weight_updates.numel() == 0
        ), "Should not have any dead neurons"
        assert (
            res.dead_encoder_bias_updates.numel() == 0
        ), "Should not have any dead neurons"
    ...
    def test_updates_a_dead_neuron_parameters(self) -> None:
        ...
        updated_parameters: ParameterUpdateResults = (
        ...
        current_dead_decoder_weights = current_parameters["_decoder._weight"][
            :, dead_neuron_idx
        ]
        updated_dead_decoder_weights = (
            updated_parameters.dead_encoder_weight_updates.squeeze()
        )
